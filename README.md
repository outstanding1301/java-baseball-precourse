...
- Baseball : Model, 숫자를 초기화
- BaseballIO : View, 입출력 관리
- BaseballController : Controller, 결과 계산 및 종료, 재시작 관리

이렇게 분리해봤는데, 이게 제대로 분리된 것인지 긴가민가 하긴 하다.  
특히 숫자 3개를 생성하는 부분이 Baseball에 들어가는 것이 과연 맞는지가 의문이다.  
Model의 상태를 변화시키는 것은 Controller가 해야 할 일인데, 초기화도 Controller가 담당하는게 맞는지 말이다.  

간단한 프로젝트인데 굳이 MVC로 만들려고 욕심을 부린 것은 아닌가 싶기도 하지만,  
